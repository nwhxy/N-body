...
__global__ void insert(int* nodeArray, int* indices, bounds* nodeBounds, float3* data, int numItems, int* supernode)
{
	...
	if(i < numItems)
	{
		...
		while(!inserted)
		{
			...
			nodeIndex = getNodeIndex(nodeBounds[currentNode], data[i]); //与当前节点索引的偏移量
			int subnodeIndex = indices[currentNode] + nodeIndex; //尝试插入的地方
			...
			else if(LEAF == atomicCAS(&nodeArray[subnodeIndex], LEAF, LOCKED))
			{
				...
				if(indices[subnodeIndex] == UNUSED)
				{
					...
					inserted = true;
					nodeArray[subnodeIndex] = LEAF; 
				}
				else
				{	
					int oldIndex = indices[subnodeIndex]; //旧的索引
...
__global__ void cuBarnesHut(int* nodeArray, int* indices, bounds* nodebounds, int* supernode, bhInfo* info, float3* pos, float3* acc, float* mass, int numItems)
{
	...
	if(ind < numItems)
	{
		extern __shared__ float3 a[];
		a[ix] = pos[ind];
	    __syncthreads();
		int currentPos = 1;
		for(int i = 0; i < 8; i++)
		{
			float3 r_ij = info[currentPos+i].centerOfMass - a[ix];
...
{
	int smem = sizeof(float3)*blockSize.x*2; 
	cuBarnesHut<<<gridSize, blockSize, smem>>>(devNodeTypes, devIndices, devNodeBounds, devSupernode, devInfo, pos, acc, mass, numItems);
